...
def load_cifar100():
    ...
    t_train_ds = tf.data.Dataset.from_tensor_slices(
        (train_data[25000:], train_labels[25000:]))
    t_test_ds = tf.data.Dataset.from_tensor_slices(
        (test_data[5000:], test_labels[5000:]))

    sh_train_ds = tf.data.Dataset.from_tensor_slices(
        (train_data[:25000], train_labels[:25000]))
    sh_test_ds = tf.data.Dataset.from_tensor_slices(
        (test_data[:5000], test_labels[:5000]))

    train_ds_size = tf.data.experimental.cardinality(train_ds).numpy()
    test_ds_size = tf.data.experimental.cardinality(test_ds).numpy()

    sh_train_ds_size = tf.data.experimental.cardinality(train_ds).numpy()
    sh_test_ds_size = tf.data.experimental.cardinality(test_ds).numpy()

    t_train_data = (t_train_ds
                    .map(process_images)
                    .shuffle(buffer_size=train_ds_size)
                    .batch(batch_size=100, drop_remainder=False))
    t_test_data = (t_test_ds
                   .map(process_images)
                   .shuffle(buffer_size=test_ds_size)
                   .batch(batch_size=10, drop_remainder=False))

    sh_train_data = (sh_train_ds
                     .map(process_images)
                     .shuffle(buffer_size=train_ds_size)
                     .batch(batch_size=100, drop_remainder=False))
    sh_test_data = (sh_test_ds
                    .map(process_images)
                    .shuffle(buffer_size=test_ds_size)
                    .batch(batch_size=10, drop_remainder=False))

    t_train_labels = train_labels[:25000].flatten()
    t_test_labels = test_labels[:25000].flatten()

    sh_train_labels = train_labels[25000:].flatten()
    sh_test_labels = test_labels[25000:].flatten()

    return t_train_data, t_train_labels, t_test_data, t_test_labels, sh_train_data, sh_train_labels, sh_test_data, sh_test_labels
...
def load_texas100():
    ...
    if not os.path.isfile(features):
        ...
        urllib.request.urlretrieve(
            "https://www.comp.nus.edu.sg/~reza/files/dataset_texas.tgz", os.path.join(base_path, 'tmp.tgz'))
    ...
    X = X[r]
    Y = Y[r]
    train_classifier_ratio, train_attack_ratio = float(
        10000)/float(X.shape[0]), 0.3
    train_target_data = X[:int(train_classifier_ratio*len_train)]
    train_attack_data = X[int(train_classifier_ratio*len_train)
                              :int((train_classifier_ratio+train_attack_ratio)*len_train)]
    ...
    train_attack_label = Y[int(train_classifier_ratio*len_train)
                               :int((train_classifier_ratio+train_attack_ratio)*len_train)]
