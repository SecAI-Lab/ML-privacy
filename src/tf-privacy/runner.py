...
if __name__ == '__main__':
    ...
    if model_type == 'alexnet':
        model = TYPE.ALEXNET(conf.input_shape, conf.classes)
    if model_type == 'densenet':
        model = TYPE.DENSENET(conf.input_shape, conf.classes)
    if model_type == 'vgg':
        model = TYPE.VGG(conf.input_shape, conf.classes)
    if model_type == 'resnet':
        model = TYPE.RESNET(conf.input_shape, conf.classes)
    ...
    logging.info(attacks_result.summary(by_slices=False))
