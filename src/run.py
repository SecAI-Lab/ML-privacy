...
if __name__ == '__main__':    
    ...
    sh_train_logits = shadow.test(shadow_path, dataloader.shadow_trainloader)
    sh_test_logits = shadow.test(shadow_path, dataloader.shadow_valloader)
   
    t_train_logits = target.test(target_path, dataloader.target_trainloader)
    t_test_logits = target.test(target_path, dataloader.target_valloader)
    ...
    if arg == 'rf':
        ...
        attack_train_data = AttackDataset(sh_train_logits, sh_test_logits)
        attack_test_data = AttackDataset(t_train_logits, t_test_logits, train=False)
        
        attacker = DistanceAttack(n_classes=conf.cifar_classes)  
        #train_attacker(attacker, attack_train_data)
